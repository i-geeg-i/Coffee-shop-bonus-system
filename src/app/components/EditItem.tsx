...
export default function EditItem({ item }: Props) {
  ...
  const handlePicChange: (e: any) => void = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!user) {
      ...
      if (error || !data) {
        ...
        if (data['is_admin']){
            ...
            .from('products')
            .update({ name: name, description: description, price: price, img: pic})
            .eq('id', item["id"])
    ...
    <>
      ...
      <div className={styles.main_block}>
        ...
        <form className={styles.right_block_edit} onSubmit={handleSubmit}>
          ...
          <input className={styles.input} value={pic} id="name" onChange={handlePicChange}></input><br />
